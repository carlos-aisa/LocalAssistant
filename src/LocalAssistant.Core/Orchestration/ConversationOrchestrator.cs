...
public sealed class ConversationOrchestrator : IConversationOrchestrator
{
    ...
    public async Task<ConversationTurnResult> ResolveConfirmationAsync(Guid id, Guid confirmationId, bool approved, ILanguageProvider provider, CancellationToken ct)
    {
        ...
        { 
            await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ConfirmationApproved, 
                                ...
                                ct); 
            var error = await ExecuteAsync(id, pending.ProviderName, pending.ToolCall, traces, e => toolsTime += e, ct); 
                                                                                                        ...
                                                                                                        CancellationToken ct)
    {
        ...
        { 
            var pending = new PendingToolConfirmation(Guid.NewGuid(), id, providerName, policyContext.PrincipalId, call, remaining, _clock.GetUtcNow().Add(_options.ConfirmationTimeout));
    ...
    private async Task<OrchestrationError?> ExecuteAsync(Guid id, string providerName, ToolCall call, List<ToolExecutionTrace> traces, Action<TimeSpan> addTime, CancellationToken ct)
    {
        ...
        try { 
            ...
            result = await tool.ExecuteAsync(call.Arguments, timeout.Token); 
        }
        catch (OperationCanceledException) when (!ct.IsCancellationRequested) 
        { 
