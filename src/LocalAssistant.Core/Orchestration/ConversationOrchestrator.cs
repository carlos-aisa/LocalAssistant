...
public sealed class ConversationOrchestrator : IConversationOrchestrator
{
    ...
    public ConversationOrchestrator(IConversationStore store, IToolRegistry tools, IToolRiskPolicy toolRiskPolicy, IToolPolicyContextAccessor toolPolicyContextAccessor, IToolAuditSink auditSink, IToolConfirmationStore confirmations, IConversationExecutionLock locks, TimeProvider clock, IOptions<OrchestrationOptions> options, ILogger<ConversationOrchestrator> logger)
    {
        _store = store; _tools = tools; _toolRiskPolicy = toolRiskPolicy; _toolPolicyContextAccessor = toolPolicyContextAccessor; _auditSink = auditSink; _confirmations = confirmations; _locks = locks; _clock = clock; _options = options.Value; _logger = logger;
    ...
    public async Task<ConversationTurnResult> ResolveConfirmationAsync(Guid id, Guid confirmationId, bool approved, ILanguageProvider provider, CancellationToken ct)
    {
        ...
        { await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ConfirmationExpired, id, pending.PrincipalId, pending.ProviderName, pending.ToolCall, "confirmation_expired", pending.ConfirmationId), ct); await RejectAsync(id, pending.ToolCall, "The tool confirmation expired.", ct); return Result(id, null, [Failed(pending.ToolCall, "confirmation_expired")], 0, TimeSpan.Zero, TimeSpan.Zero, new("confirmation_expired", "The confirmation expired.")); }
        var traces = new List<ToolExecutionTrace>(); var toolsTime = TimeSpan.Zero;
        if (approved)
        { await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ConfirmationApproved, id, pending.PrincipalId, pending.ProviderName, pending.ToolCall, confirmationId: pending.ConfirmationId), ct); var error = await ExecuteAsync(id, pending.ProviderName, pending.ToolCall, traces, e => toolsTime += e, ct); if (error is not null) return Result(id, null, traces, 0, TimeSpan.Zero, toolsTime, error); }
        else { await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ConfirmationRejected, id, pending.PrincipalId, pending.ProviderName, pending.ToolCall, "tool_confirmation_rejected", pending.ConfirmationId), ct); await RejectAsync(id, pending.ToolCall, "The user rejected this tool call.", ct); traces.Add(Failed(pending.ToolCall, "tool_confirmation_rejected")); }
        foreach (var call in pending.RemainingToolCalls)
        { var outcome = await HandleAsync(id, provider.Name, policyContext, call, [], traces, e => toolsTime += e, ct); if (outcome.Error is not null || outcome.Confirmation is not null) return Result(id, null, traces, 0, TimeSpan.Zero, toolsTime, outcome.Error, outcome.Confirmation); }
        return await ContinueAsync(id, provider, policyContext, traces, 0, toolsTime, ct);
    }

    private async Task<ConversationTurnResult> ContinueAsync(Guid id, ILanguageProvider provider, ToolPolicyContext policyContext, List<ToolExecutionTrace> traces, int completedIterations, TimeSpan toolsTime, CancellationToken ct)
    {
        ...
        for (var iteration = completedIterations + 1; iteration <= _options.MaxIterations; iteration++)
        {
            ...
            try { OrchestrationLog.ProviderCalled(_logger, provider.Name, iteration, id); using var timeout = CancellationTokenSource.CreateLinkedTokenSource(ct); timeout.CancelAfter(_options.ProviderTimeout); response = await provider.GetResponseAsync(new(id, await _store.GetMessagesAsync(id, ct), GetAvailableDefinitions(policyContext)), timeout.Token); }
            catch (OperationCanceledException) when (!ct.IsCancellationRequested) { providerTime += _clock.GetElapsedTime(start); OrchestrationLog.ProviderTimedOut(_logger, provider.Name, iteration, id); return Result(id, null, traces, iteration, providerTime, toolsTime, new("provider_timeout", "The language provider timed out.")); }
            ...
            { var outcome = await HandleAsync(id, provider.Name, policyContext, response.ToolCalls[index], response.ToolCalls.Skip(index + 1).ToArray(), traces, e => toolsTime += e, ct); if (outcome.Error is not null || outcome.Confirmation is not null) return Result(id, null, traces, iteration, providerTime, toolsTime, outcome.Error, outcome.Confirmation); }
        }
    ...
    private async Task<(OrchestrationError? Error, ToolConfirmationRequest? Confirmation)> HandleAsync(Guid id, string providerName, ToolPolicyContext policyContext, ToolCall call, IReadOnlyList<ToolCall> remaining, List<ToolExecutionTrace> traces, Action<TimeSpan> addTime, CancellationToken ct)
    {
        ...
        if (!_tools.TryGet(call.Name, out var tool) || tool is null) { await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ToolNotFound, id, policyContext.PrincipalId, providerName, call, "tool_not_found"), ct); traces.Add(Failed(call, "tool_not_found")); return (new("tool_not_found", "The requested tool is not registered.", call.Name), null); }
        ...
        { var pending = new PendingToolConfirmation(Guid.NewGuid(), id, providerName, policyContext.PrincipalId, call, remaining, _clock.GetUtcNow().Add(_options.ConfirmationTimeout)); await _confirmations.CreateAsync(pending, ct); await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ConfirmationRequested, id, policyContext.PrincipalId, providerName, call, confirmationId: pending.ConfirmationId), ct); return (null, new(pending.ConfirmationId, call.Id, call.Name, call.Arguments, pending.ExpiresAtUtc)); }
        return (await ExecuteAsync(id, providerName, call, traces, addTime, ct), null);
    }

    private async Task<OrchestrationError?> ExecuteAsync(Guid id, string providerName, ToolCall call, List<ToolExecutionTrace> traces, Action<TimeSpan> addTime, CancellationToken ct)
    {
        var policyContext = _toolPolicyContextAccessor.GetCurrent();
        if (!_tools.TryGet(call.Name, out var tool) || tool is null) { await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ToolNotFound, id, policyContext.PrincipalId, providerName, call, "tool_not_found"), ct); traces.Add(Failed(call, "tool_not_found")); return new("tool_not_found", "The requested tool is not registered.", call.Name); }
        var policyDecision = _toolRiskPolicy.Evaluate(tool.Definition.Metadata, policyContext);
        if (policyDecision.Kind == ToolPolicyDecisionKind.Denied)
        {
            await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.PolicyDenied, id, policyContext.PrincipalId, providerName, call, policyDecision.Code ?? "tool_policy_denied"), ct);
        ...
        catch (OperationCanceledException) when (!ct.IsCancellationRequested) { var elapsed = _clock.GetElapsedTime(start); addTime(elapsed); await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ExecutionTimedOut, id, policyContext.PrincipalId, providerName, call, "tool_timeout", durationMilliseconds: elapsed.TotalMilliseconds), ct); traces.Add(Failed(call, "tool_timeout", elapsed)); OrchestrationLog.ToolFailed(_logger, call.Name, call.Id, "tool_timeout", id); return new("tool_timeout", "The tool timed out.", call.Name); }
        catch (Exception exception) when (exception is not OperationCanceledException) { var elapsed = _clock.GetElapsedTime(start); addTime(elapsed); await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ExecutionFailed, id, policyContext.PrincipalId, providerName, call, "tool_execution_failed", durationMilliseconds: elapsed.TotalMilliseconds), ct); traces.Add(Failed(call, "tool_execution_failed", elapsed)); OrchestrationLog.ToolFailed(_logger, call.Name, call.Id, "tool_execution_failed", id, exception); return new("tool_execution_failed", "The tool failed during execution.", call.Name); }
        ...
        if (!result.IsSuccess) { var code = result.ErrorCode ?? "tool_execution_failed"; await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ExecutionFailed, id, policyContext.PrincipalId, providerName, call, code, durationMilliseconds: duration.TotalMilliseconds), ct); traces.Add(Failed(call, code, duration)); OrchestrationLog.ToolFailed(_logger, call.Name, call.Id, code, id); return new(code, result.ClientMessage ?? "The tool failed.", call.Name); }
        await WriteAuditAsync(CreateAuditEvent(ToolAuditEventType.ExecutionSucceeded, id, policyContext.PrincipalId, providerName, call, durationMilliseconds: duration.TotalMilliseconds), ct);
        traces.Add(new(call.Id, call.Name, true, duration.TotalMilliseconds)); OrchestrationLog.ToolCompleted(_logger, call.Name, call.Id, duration.TotalMilliseconds, id); return null;
    }
    ...
    { var now = _clock.GetUtcNow(); var result = new ConversationTurnResult(id, content, traces, iterations, new(now, now, providerTime.TotalMilliseconds + toolsTime.TotalMilliseconds, providerTime.TotalMilliseconds, toolsTime.TotalMilliseconds), error, confirmation); OrchestrationLog.TurnCompleted(_logger, id, iterations, result.Timings.TotalMilliseconds); return result; }
