...
public sealed class ConversationOrchestrator : IConversationOrchestrator
{
    ...
    public ConversationOrchestrator(IConversationStore store, IToolRegistry tools, IToolRiskPolicy toolRiskPolicy, IToolPolicyContextAccessor toolPolicyContextAccessor, IToolConfirmationStore confirmations, IConversationExecutionLock locks, TimeProvider clock, IOptions<OrchestrationOptions> options, ILogger<ConversationOrchestrator> logger)
    {
        _store = store; _tools = tools; _toolRiskPolicy = toolRiskPolicy; _toolPolicyContextAccessor = toolPolicyContextAccessor; _confirmations = confirmations; _locks = locks; _clock = clock; _options = options.Value; _logger = logger;
    ...
    private async Task<ConversationTurnResult> ContinueAsync(Guid id, ILanguageProvider provider, ToolPolicyContext policyContext, List<ToolExecutionTrace> traces, int completedIterations, TimeSpan toolsTime, CancellationToken ct)
    {
        ...
        for (var iteration = completedIterations + 1; iteration <= _options.MaxIterations; iteration++)
        {
            ...
            try { using var timeout = CancellationTokenSource.CreateLinkedTokenSource(ct); timeout.CancelAfter(_options.ProviderTimeout); response = await provider.GetResponseAsync(new(id, await _store.GetMessagesAsync(id, ct), GetAvailableDefinitions(policyContext)), timeout.Token); }
    ...
    private async Task<OrchestrationError?> ExecuteAsync(Guid id, ToolCall call, List<ToolExecutionTrace> traces, Action<TimeSpan> addTime, CancellationToken ct)
    {
        ...
        var policyDecision = _toolRiskPolicy.Evaluate(
